...
questions from interested peole wondering what my impressions and experiences with
the device.
...
* Sleep doesn't work. At all.
  * There is apparently a serious firmware bug in the Trusted Firmeware Alliance) boot loader that most PBPro distros use, so even if you do fix your lid magnet, the laptop will still drain its battery overnight if you close the lid and walk away. Not amazing.
...
* Don't assume that just because your favorite flavor/distro exists that it'll be at all usable or run well
  * Rolling an SD boot image for the Pinebook Pro isn't particularly hard, which means a LOT of people have tried their hand at it with mixed results. The Pinebook Pro's unique big/little ARM CPU arrangement requires some fairly specific kernel tuning to perforrm well, so I'd strongly recommend running one of the stock distros (Currently Debian or Manjaro) unless you're into that kind of super low level hackery :)
...
Over the last 3 months I have learned an incredible amount about ARM System on a Chip architectures - how their firmware is laid out, how the boot process differens from X86 (TL;DR - A LOT!) and a host of other tidbits. It's not every day you get to explore an entirely new architecture, and that has been surprisingly edifying.
...
* Spring for the extra $50 128GB emmc. It's a trivial price increment to double your storage without all the footnotes and question marks around adding an NVME SSD (There are power drain and compatability issues).
